...
class TrellisNet(Layer):
    """Creates a TrellisNet layer.
        ...
        Args:
            ...
            kwargs: Any other arguments for configuring parent class Layer. For example "name=str", Name of the model.
                    Use unique names when using multiple TrellisNet.
    ...
    def __init__(self,
        ...
        super(TrellisNet, self).__init__(**kwargs)
    ...
    def get_config(self):
        ...
        """
        config = super(TrellisNet, self).get_config()
...
def compiled_TrellisNet(num_feat,  # type: int
                 ...
                 use_weight_norm=False):
    # type: (...) -> Model
    """Creates a compiled TrellisNet model for a given task (i.e. regression or classification).
    ...
    Args:
        ...
        dropout_rate: Float between 0 and 1. Fraction of the input units to drop.
        activation: The activation used in the residual blocks o = Activation(x + F(x)).
        name: Name of the model. Useful when having multiple TrellisNet.
        kernel_initializer: Initializer for the kernel weights matrix (Conv1D).
        opt: Optimizer name.
        lr: Learning rate.
        use_batch_norm: Whether to use batch normalization in the residual layers or not.
    ...
    x = TrellisNet(nb_filters, kernel_size, nb_stacks, dilations, padding,
...
def TrellisNet_full_summary(model: Model, expand_residual_blocks=True):
    ...
    if versions[0] <= 2 and versions[1] < 5:
        ...
        for i in range(len(layers)):
            if isinstance(layers[i], TrellisNet):
        ...
        print('WARNING: Compatible with tensorflow 2.5.0 or below.')
